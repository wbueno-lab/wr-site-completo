...
interface JacketBrandFilterProps {
  brands: Brand[];
...
const JacketBrandFilter = ({ 
  brands, 
  selectedBrands, 
  onBrandChange,
  isLoading = false
}: JacketBrandFilterProps) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [filteredBrands, setFilteredBrands] = useState<Brand[]>([]);

  // Filtrar apenas marcas que fazem jaquetas
  useEffect(() => {
    ...
    if (!searchQuery.trim()) {
      setFilteredBrands(jacketBrands);
    } else {
      const filtered = jacketBrands.filter(brand =>
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Marcas de Jaquetas</CardTitle>
        </CardHeader>
    ...
    <Card>
      <CardHeader>
        ...
        <div className="relative">
          ...
          <Input
            id="jacket-brand-search"
            name="jacket-brand-search"
            placeholder="Buscar marcas de jaquetas..."
            value={searchQuery}
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {filteredBrands.length === 0 ? (
            <div className="text-center py-4 text-muted-foreground text-sm">
              {searchQuery ? 'Nenhuma marca de jaqueta encontrada' : 'Carregando marcas de jaquetas...'}
            </div>
          ) : (
...
export default JacketBrandFilter;
